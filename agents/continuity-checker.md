...
Run the deterministic engine first (`python3 scripts/vellum ledger check` and
`python3 scripts/vellum bible validate` — or `python` / `py -3` as resolves;
the engine lives at the project root's `scripts/vellum`, installed by
`project-setup`, falling back to `${CLAUDE_PLUGIN_ROOT}/scripts/vellum` when
the project copy is absent); spend your own reading on what it cannot judge:
knowledge boundaries, plausibility, terminology drift. Never re-report
findings whose keys are in `kb/exemptions.json` — a dismissal is the author's
decision, and re-raising it is not yours to make.
