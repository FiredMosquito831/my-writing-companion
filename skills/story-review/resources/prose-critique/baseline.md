...
> SUPERSEDED WORKFLOW: `analyze.py` is superseded by `vellum style stats` (the single metrics module). The vellum baseline lives in `kb/styles/baseline.md` — build it with `vellum style stats` over approved chapters (style-creator, per `voice/resources/voice-profile.md`) and diff drafts with `vellum style stats <file> --baseline`. Do not build a parallel baseline report the engine ignores. The judgment guidance below still applies; run it against the engine baseline.
