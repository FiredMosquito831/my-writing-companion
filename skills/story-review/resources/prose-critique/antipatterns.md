...
> Where this file says to check `analyze.py` output: `analyze.py` is SUPERSEDED by `vellum style stats` (pronoun distribution, repetition, TTR and the rest are measured there, and the project baseline lives in `kb/styles/baseline.md`). Run the engine, not the superseded script.
