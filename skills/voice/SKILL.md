...
- `resources/character-dialogue-profiles.md` — the per-character analog of the blind-tag test: the optional character-page `dialogue profile` section (idiolect markers, preferred deflections, sentence-length signature, top tokens) built from `@character-sim` at page creation, and the ~70% blind speaker-attribution test on attribution-stripped dialogue lines; the mechanical companion is `vellum style stats --dialogue` (per-speaker stats + pairwise convergence flags, report-only).
